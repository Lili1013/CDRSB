...
if __name__ == '__main__':
    # #--------first step------------
    # data_path = '/data/lwang9/datasets/Douban/book/douban_movie.tsv'
    # data_path = '/data/lwang9/datasets/Douban/movie/douban_movie.tsv'
    data_path = '/data/lwang9/datasets/Douban/movie/douban_movie.tsv'
    df = pd.read_csv(data_path,sep='\t')
    print(len(df))
    data = filter_g_k_one(df,k=20,u_name='UserId',i_name='ItemId',y_name='Rating')
    print(len(data))
    all_users = list(data['UserId'].unique())
    sample_users = random.sample(all_users,5000)
    data_new = data[data['UserId'].isin(sample_users)]
    data_new = filter_g_k_one(data_new, k=20, u_name='UserId', i_name='ItemId', y_name='Rating')
    print('gg')

    df_social = pd.read_csv('/data/lwang9/datasets/Douban/socialnet/socialnet.tsv',sep='\t')
    # inters_users = list(data['UserId'].unique())
    # # social_users = list(df_social['Followee'].unique())
    # social_users = list(df_social['Follower'].unique())
    # intersection_users = list(set(inters_users) & set(social_users))
    # data_new = data[data['UserId'].isin(intersection_users)]
    data_new.rename(columns={'UserId': 'user_id', 'ItemId': 'item_id','Rating':'rating'}, inplace=True)

    # df_social_new = df_social[(df_social['Followee'].isin(intersection_users))&(df_social['Follower'].isin(intersection_users))]
    df_social_new = df_social[
        (df_social['Follower'].isin(sample_users)) & (df_social['Followee'].isin(sample_users))]
    data = map_user_item_id(data_new,user_path='../datasets/douban_movie/user_id_map.pickle',
                            item_path='../datasets/douban_movie/item_id_map.pickle')
    data.to_csv('../datasets/douban_movie/douban_movie_data.csv', index=False)
    # df_social_new.to_csv('../datasets/douban_movie/douban_music_social_data.csv',index=False)
    # #------------second step: to save time, directly read file-----------

    process_user_item_interactions(data,to_path='../datasets/douban_movie/user_purd_items.pickle')
    process_item_user_interactions(data,to_path='../datasets/douban_movie/item_purd_users.pickle')
    process_user_social(df_social_new,path_map='../datasets/douban_movie/user_id_map.pickle',
                        to_path_1='../datasets/douban_movie/douban_movie_social_data.csv',
                        to_path_2='../datasets/douban_movie/user_social.pickle')
    process_rating(data,path='../datasets/douban_movie/user_purd_items.pickle',
                   path_to='../datasets/douban_movie/user_item_rating.pickle')
    process_rating(data,path='../datasets/douban_movie/item_purd_users.pickle',
                   path_to = '../datasets/douban_movie/item_user_rating.pickle')
    split_datasets_rating(data, train_path='../datasets/douban_movie/train.csv',
                          test_path='../datasets/douban_movie/test.csv')
    # ranking split datasets
    split_datasets_ranking(data,to_path='../datasets/douban_movie/douban_movie_inter.csv')
