...
def generate_test_negatives(index_range, df_all, df_test, df_train, test_neg_num, num_items):
    ...
    while i < index_range[1]:
        ...
        uid = row['user_id']
        iid = row['item_id']
        red_by_f = row['red_by_friends']
        each_negatives.append(int(uid))
        each_negatives.append(int(iid))
        each_negatives.append(int(red_by_f))
        for t in range(num_negatives):
            j = np.random.randint(num_items)
...
if __name__ == '__main__':
    ...
    path = '../datasets/yelp/yelp_inter_new.csv'
    df = pd.read_csv(path)
    train_data = df[df['x_label'] == 0][['user_id', 'item_id', 'rating','red_by_friends']]
    test_data = df[df['x_label'] == 1][['user_id', 'item_id', 'rating','red_by_friends']]
    print(len(test_data))
    generate_and_store_test_negatives(df, test_data, train_data, 99,'../datasets/yelp/yelp_test.txt')
